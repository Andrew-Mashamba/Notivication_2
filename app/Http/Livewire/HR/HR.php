...
class HR extends Component
{
    ...
    public function editDepartment(){
        ...
        try {
            ...
            $apprpvals->sendApproval(session()->get('departmentIdForEditing')," Update$this->department_name ","has updated $this->department_name role","has update $this->department_name role","102",'','');
    ...
    public function departmentAction(){
        ...
        if($checkPassword){
            $department=DB::table('roles')->where('id',session()->get('departmentIdToAction'))->first();
            $prev_status=$department->status;
            if($this->permission==$prev_status){
                session()->flash('message_fail',"You cannot $this->permission twice");
            }
            else{
                DB::table('roles')->where('id',session()->get('departmentIdToAction'))->update([ 'status'=>$this->permission]);

                // send for the approvals
                $apprpvals=new approvals();
                $apprpvals->sendApproval($department->id,"$this->permission Role","has $this->permission department","has to $this->permission department","102",'');
    ...
    public function departmentEdit(){
        if($this->openEditDepartmentModal==false){
            ...
            $this->department_name=DB::table('roles')->where('id',session()->get('departmentIdForEditing'))->value('department_name');
        }
