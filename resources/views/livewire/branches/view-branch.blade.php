                    ...
                    <form>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                ...
                                <div class="bg-gray-100 p-4 rounded-3xl">
                                    <h3 class="text-lg font-semibold mb-2">Region</h3>
                                    <p class="text-gray-600 text-xl font-bold">
                                        {{\Illuminate\Support\Facades\DB::table('regions')->where('id',$branch->region)->value('region_name')}}
                                    </p>
                                </div>

                                <!-- Total Value of Pledged Stock -->
                                <div class="bg-gray-100 p-4 rounded-3xl">
                                    <h3 class="text-lg font-semibold mb-2">District</h3>
                                    <p class="text-gray-600">
                                        {{\Illuminate\Support\Facades\DB::table('districts')->where('id',$branch->district)->value('district_name')}}
                                    </p>
                                </div>

                                <!-- Visual Representation of Stock Distribution (Placeholder) -->
                                <div class="bg-gray-100 p-4 rounded-3xl">
                                    <h3 class="text-lg font-semibold mb-2">Ward</h3>
                                    <p class="text-gray-600">
                                        {{\Illuminate\Support\Facades\DB::table('wards')->where('id',$branch->ward)->value('ward_name')}}
                                    </p>
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="flex flex-wrap">
                            @foreach(\Illuminate\Support\Facades\DB::table('users')->where('branch', $branch->id )->get() as $user)
                                @if($user)
                                ...
                                <span class="flex items-center justify-center w-14 h-14 shrink-0 rounded-full bg-white yellow">
                                    <img src="{{ asset('images/avatar.png') }}" class="rounded-full h-12" alt="Avatar Image" >
                                        ...
                                        <div class="flex flex-col flex-1">
                                            ...
                                            <div class="">
                                            <div class="inline-block px-3 text-xs leading-none text-gray-400 font-normal first:pl-0">
                                                {{\Illuminate\Support\Facades\DB::table('roles')->where('id',$user->role)->value('department_name')}}
                                            </div>
                                                <div class="inline-block px-3 text-xs leading-none text-gray-400 font-normal first:pl-0">
                                                    {{$user->email}}
                                                </div>
