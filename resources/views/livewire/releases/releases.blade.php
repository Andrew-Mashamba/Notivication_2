            ...
            <div class="flex items-center justify-center min-h-screen bg-gray-200 p-2">
                <div aria-label="card" class="p-6 rounded-3xl bg-white w-full min-h-screen">
                    ...
                    <div aria-label="content" class="mt-9 grid gap-2.5">
                        ...
                        @foreach(\Illuminate\Support\Facades\DB::table('releases')->where('Status','PENDING')->get() as $client)
                            <a wire:click="loadView({{$client->id}})" class="cursor-pointer">
                                ...
                                <span class="flex items-center justify-center w-14 h-14 shrink-0 rounded-full bg-white yellow">
                                    ...
                                    <svg class="rounded-3xl bg-gray-100 p-1 h-10"  data-slot="icon" fill="none" stroke-width="1.5" stroke="#A8A8A8" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                                      <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"></path>
                                    </svg>
                                </span>
                                    <div class="w-full">
                                        <h3 class="text-sm font-medium"><p class="text-red-600 text-sm font-semibold">NEW REQUEST</p></h3>
                                        ...
                                        <h3 class="text-sm font-medium">
                                            Amount : {{number_format(\Illuminate\Support\Facades\DB::table('applications')->where('id',$client->application_id)->value('loan_amount'))}}
                                            TZS</h3>
                        ...
                        <hr class="mt-2 mb-2 border-b-1 border-blueGray-300">
                        @foreach(\Illuminate\Support\Facades\DB::table('releases')->where('Status','COLLATERAL RELEASED')->get() as $client)
                            <a wire:click="loadView({{$client->id}})" class="cursor-pointer">
                                <div class="flex items-center space-x-4 p-2 rounded-3xl bg-gray-100">
                                <span class="flex items-center justify-center w-14 h-14 shrink-0 rounded-full bg-white yellow">
                                    ...
                                    <div class="w-full">
                                        <h3 class="text-sm font-medium"><p class="text-blue-600 text-sm font-semibold">COLLATERAL RELEASED</p></h3>
                                        <h3 class="text-sm font-medium">Name :
                                            {{\Illuminate\Support\Facades\DB::table('farmers')->where('id',$client->client_id)->value('first_name')}}
                                            {{\Illuminate\Support\Facades\DB::table('farmers')->where('id',$client->client_id)->value('last_name')}}
                                        </h3>
                        ...
                        <hr class="mt-2 mb-2 border-b-1 border-blueGray-300">
                        @foreach(\Illuminate\Support\Facades\DB::table('releases')->where('Status','COLLATERAL RELEASE REQUESTED')->get() as $client)
                            <a wire:click="loadView({{$client->id}})" class="cursor-pointer">
                                <div class="flex items-center space-x-4 p-2 rounded-3xl bg-gray-100">
                                <span class="flex items-center justify-center w-14 h-14 shrink-0 rounded-full bg-white yellow">
                                    ...
                                    <div class="w-full">
                                        <h3 class="text-sm font-medium"><p class="text-blue-600 text-sm font-semibold">COLLATERAL RELEASE REQUESTED</p></h3>
                                        <h3 class="text-sm font-medium">Name :
                                            {{\Illuminate\Support\Facades\DB::table('farmers')->where('id',$client->client_id)->value('first_name')}}
                                            {{\Illuminate\Support\Facades\DB::table('farmers')->where('id',$client->client_id)->value('last_name')}}
                                        </h3>
                    ...
                    <div aria-label="content" class="mt-9 grid gap-2.5">
                        ...
                        <div wire:loading.remove wire:target="loadView">
                            @if($this->viewLoaded == "a")
                                <livewire:releases.new-release />
                            @elseif($this->viewLoaded == "b")
                                <livewire:releases.dashboard />
                            @else

                                <livewire:releases.view-release />
                            @endif
                        </div>
