...
class ShowCommand extends DatabaseInspectionCommand
{
    ...
    protected function displayForCli(array $data)
    {
        ...
        $this->components->twoColumnDetail('<fg=blue;options=bold>'.$platform['name'].'</>');
        ...
        if ($tables->isNotEmpty()) {
            $this->components->twoColumnDetail('<fg=blue;options=bold>Table</>', 'Size (MiB)'.($this->option('counts') ? ' <fg=gray;options=bold>/</> <fg=yellow;options=bold>Rows</>' : ''));
        ...
        if ($views && $views->isNotEmpty()) {
            $this->components->twoColumnDetail('<fg=blue;options=bold>View</>', '<fg=blue;options=bold>Rows</>');
