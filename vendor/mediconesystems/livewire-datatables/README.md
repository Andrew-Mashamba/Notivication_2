...
{
    return 'divide-x divide-gray-100 text-sm yellow ' . ($this->rowIsSelected($row) ? 'bg-yellow-100' : ($row->{'car.model'} === 'Ferrari' ? 'bg-red-500' : ($loop->even ? 'bg-gray-100' : 'bg-gray-50')));
}

public function cellClasses($row, $column)
{
    return 'text-sm ' . ($this->rowIsSelected($row) ? ' yellow' : ($row->{'car.model'} === 'Ferrari' ? ' text-white' : ' yellow'));
}
```
