...
>
    ...
    @if(count($this->rules[0]['content']))
        <div class="my-4 px-4 py-2 bg-gray-500 whitespace-pre-wrap @if($errors->any())text-red-200 @else text-blue-100 @endif rounded">{{ $this->rulesString }}@if($errors->any()) Invalid rules @endif</div>
    @endif
                ...
                <div class="mt-2 sm:mt-0 sm:flex sm:space-x-2">
                    @isset($savedQueries)
                        <div class="flex items-center space-x-2" x-data="{
                            ...
                            <input x-model="name" wire:loading.attr="disabled" x-on:keydown.enter="saveQuery" placeholder="save as..." class="flex-grow px-3 py-3 border text-sm yellow leading-4 block rounded-md border-gray-300 shadow-sm focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" />
                            <button x-bind:disabled="! name" x-show="rules" x-on:click="saveQuery" class="flex items-center space-x-2 px-3 py-0.5 border border-blue-400 disabled:border-gray-300 rounded-md bg-white text-blue-500 disabled:text-gray-300 text-xs leading-4 font-medium uppercase tracking-wider hover:bg-blue-200 disabled:hover:bg-white focus:outline-none disabled:pointer-events-none">
                                <span>{{ __('Save') }}</span>
