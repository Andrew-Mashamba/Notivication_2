 ...
 *
 * named = false
 * | 1   |    {/list}    /red,blue,blue                  | {$value}*(?:,{$value}+)*
 * | 2   |    {/list*}   /red/blue/blue                  | {$value}+(?:{$sep}{$value}+)*
 * | 3   |    {/keys}    /semi,%3B,dot,.,comma,%2C        | /(\w+,?)+
 * | 4   |    {/keys*}   /semi=%3B/dot=./comma=%2C        | /(?:\w+=\w+/?)*
 * named = true
 * | 1   |    {?list}    ?list=red,blue,blue             | {name}=(?:\w+(?:,\w+?)*)*
 * | 2   |    {?list*}   ?list=red&list=blue&list=blue   | {name}+=(?:{$value}+(?:{sep}{name}+={$value}*))*
